...
/*
 * M55 HP <-> A32 Communication Test (MHU1)
 *
                    ...
                    uint32_t id, volatile void *data)
{
    ...
    /* Print received data */
    printk("M55-HP: RX MHU1 Ch%d = 0x%x\n", id, *((uint32_t *)data));
    msg_received = true;
}
                    ...
                    uint32_t id, volatile void *data)
{
    ...
    printk("M55-HP: TX MHU1 Ch%d Done\n", id);
    msg_sent = true;
}
...
static void send_reply(void)
{
     ...
     * A32 Linux should listen on "rxdb1" (M55 HP MHU1 RX endpoint).
     */

    /* Send 1st Message */
    msg_sent = false;
    tx_msg = 0xdead1100;  // Different data pattern for HP MHU1
    printk("M55-HP: Sending 0x%x on MHU1 Ch0...\n", tx_msg);
    ipm_send(mhu1_s, 0, 0, &tx_msg, 4);
    while (!msg_sent);

    /* Send 2nd Message */
    msg_sent = false;
    tx_msg = 0xdead1101;  // Different data pattern for HP MHU1
    printk("M55-HP: Sending 0x%x on MHU1 Ch0...\n", tx_msg);
    ipm_send(mhu1_s, 0, 0, &tx_msg, 4);
    while (!msg_sent);
}
...
int main(void)
{
    ...
    printk("\n========================================\n");
    printk("M55-HP <-> A32 (APSS) MHU1 Test\n");
    printk("========================================\n");
