 ...
 *
 * MHU Doorbell + Shared SRAM1 test: M55-HP <-> A32
 *
 * Protocol:
 *   - SRAM1 region at 0x027DDA00 is used as shared memory.
 ...
 *   A32 writes to SRAM via non-cacheable /dev/mem mapping, but M55-HP
 *   has D-cache enabled. Before reading shared SRAM written by A32,
 *   M55-HP must invalidate its D-cache for that region. After writing
 *   shared SRAM for A32, M55-HP must clean (flush) its D-cache.
 *
 * Shared memory layout (struct shared_msg at SHARED_MEM_BASE):
 *   [0x00] magic      - 0xA32F055E when written by A32, 0xF055EA32 by HP
 ...
 *   1. HP waits for doorbell from A32 (MHU RX interrupt)
 *   2. HP invalidates D-cache, then reads shared SRAM
 *   3. HP writes a response into shared SRAM, then flushes D-cache
 *   4. HP rings doorbell to A32 (MHU TX) with the response address
 */
 ...
 * Available because __DCACHE_PRESENT=1 in soc.h for M55-HP */
...
#define A32_TO_HP_ADDR   (SHARED_MEM_BASE + 0x0000)  /* A32 writes here */
#define HP_TO_A32_ADDR   (SHARED_MEM_BASE + 0x0100)  /* HP writes here  */
...
const struct device *mhu_rx_dev;  /* MHU1 receiver from A32 */
const struct device *mhu_tx_dev;  /* MHU1 sender to A32     */
			    ...
			    uint32_t id, volatile void *data)
{
	...
	printk("M55-HP: Doorbell RX Ch%d, addr=0x%08x\n", id, rx_doorbell_addr);
...
int main(void)
{
	...
	volatile struct shared_msg *tx_msg =
			(volatile struct shared_msg *)HP_TO_A32_ADDR;

	printk("\n==========================================\n");
	printk("M55-HP <-> A32 : MHU1 Doorbell + Shared SRAM1\n");
	printk("  Shared RX (A32->HP): 0x%08x\n", A32_TO_HP_ADDR);
	printk("  Shared TX (HP->A32): 0x%08x\n", HP_TO_A32_ADDR);
	printk("==========================================\n");
	...
	/*
	 * Only clear HP's own TX region.
	 * Do NOT clear A32_TO_HP region — A32 may have already written data
	 * there before HP boots.
	 */
	memset((void *)HP_TO_A32_ADDR, 0, sizeof(struct shared_msg));
	CACHE_CLEAN(HP_TO_A32_ADDR, sizeof(struct shared_msg));
	...
	for (iter = 0; iter < ITERATIONS; iter++) {
		...
		if (rx_doorbell_addr != A32_TO_HP_ADDR) {
			printk("ERROR: Unexpected doorbell addr 0x%08x "
			       "(expected 0x%08x)\n",
			       rx_doorbell_addr, A32_TO_HP_ADDR);
		...
		uint32_t computed_cksum = calc_checksum((const uint8_t *)incoming->data,
							rx_len);
		printk("[%d] A32->HP: msg_id=%u len=%u cksum=0x%x/0x%x %s\n",
		...
		CACHE_INVALIDATE(HP_TO_A32_ADDR, sizeof(struct shared_msg));
		__DSB();

		/* --- Write response into shared SRAM --- */
		tx_msg->magic    = MAGIC_HP;
		tx_msg->msg_id   = iter;
		...
		printk("[%d] HP->A32: Sending response at 0x%08x\n",
		       iter, HP_TO_A32_ADDR);

		/* --- Ring doorbell to A32 with address of response --- */
		doorbell_tx_done = false;
		uint32_t doorbell_val = HP_TO_A32_ADDR;
